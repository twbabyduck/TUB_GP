...
def _Kfu_fallback_multioutput(Z, kern, X, **kwargs):
    ...
    # Swap the batch axes of x and z
    # keep L output-features first
    axes = list(range(1, ndims_x + ndims_z + 1))
...
def _Kfu_fallback_shared(Z, kern, X, **kwargs):
    # Edge-case where L is supressed
    return _Kfu_fallback(Z, kern, X, **kwargs)
...
def _Kfu_conv2d_fallback(feat: InducingImages,
    ...
    if full_spatial:  # convert to 4D image format
        spatial_out = kern.get_spatial_out(
            Xnew.shape[-3:-1])  # to [N, H, W, M]
...
def _Kfu_conv2d(feat: InducingImages,
    ...
    if not full_spatial:
        Kxz = tf.reshape(
            Kxz, list(Kxz.shape[:-3]) + [-1, len(feat)])  # [N, P, M]
        if kern.weights is None:
...
def _Kfu_depthwise_conv2d_fallback(feat: DepthwiseInducingImages,
    ...
    Kxz = kern.kernel.K_r2(r2)
    # convert to 4D image format as [N, H, W, channels_in * M]
    if full_spatial:
...
def _Kfu_depthwise_conv2d(feat: DepthwiseInducingImages,
    ...
    if kern.kernel.ard:  # notice the transpose!
        assert tuple(precis.shape) == (
            channels_in, tf.reduce_prod(patch_shape))
        filters = tf.reshape(tf.transpose(
            precis), patch_shape + [channels_in, 1])
    else:
        filters = tf.fill(patch_shape + [channels_in, 1], precis)
    ...
    if full_spatial:
        ...
        if kern.weights is None:
            ...
            Kxz = div * \
                tf.tensordot(Kxz, tf.reshape(kern.weights, [-1]), [-2, -1])
