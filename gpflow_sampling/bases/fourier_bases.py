...
class Dense(AbstractFourierBasis):
    ...
    def initialize(self, x: TensorType, dtype: Any = None):
        ...
        self._biases = bias_initializer(
            self.kernel, self.num_bases, dtype=dtype)
...
class MultioutputDense(Dense):
    def __call__(self, x: TensorType, multioutput_axis: int = None, **kwargs):
        ...
        batch_axes = None if (multioutput_axis is None) else [
            0, multioutput_axis]
        ...
        # [L, N, B]
        return expand_to(self.output_scale, axis=1, ndims=ndims) * feat
...
class Conv2d(AbstractFourierBasis):
    ...
    def initialize(self, x, dtype: Any = None):
        ...
        shape = self.kernel.patch_shape + \
            [self.kernel.channels_in, self.num_bases]
